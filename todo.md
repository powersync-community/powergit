...
- Explorer Git pack indexing now routes `.pack` IO through the new `PackFileStore` inside `PowerSyncFs`, so LightningFS/IndexedDB is gone and we only mirror PowerSync’s pack bytes once. Give the `/org/:orgId/repo/:repoId/files` view a fresh clone to confirm reads still hydrate from the store, then remove the debug logging once we’re confident the backend holds up (`packages/apps/explorer/src/ps/git-store.ts`).
...
- Stream orchestration needs a refresh: the explorer and sync rules now pass `{ org_id, repo_id }` parameters for each stream, but the daemon server still exposes plain string lists and `startDaemon` doesn’t wire `subscribeStreams`/`unsubscribeStreams`. Git helper / CLI can’t resubscribe, leaving the synced tables empty client-side.
...
- Explorer bootstrap used to call `SELECT powersync_disable_drop_view()` before the raw-table migration; with the migration removed we no longer need the toggle, but keep the patched wasm/dylib handy in case we have to reintroduce it. The patched core still includes:
...
- Docs/diagrams still call out raw-table plumbing; refresh the github-import flow + CLI messaging so they describe the default PowerSync views and the absence of `raw_*` mirrors.
...
- Removed the bespoke raw-table bootstrap/migration path — the explorer and daemon now rely on PowerSync’s default view-backed tables. Need to monitor first-run upgrades on existing browsers (verify `ps_untyped` stays empty) and tighten typings around the new schema helpers once the explorer TypeScript errors are addressed.
...
- Completed the migration away from bespoke raw tables: shared schema now feeds the default PowerSync views end-to-end, Supabase mirrors the same columns/indexes, and the daemon/explorer consume the managed replica without auxiliary views or migrations.
- Updated the remote helper push path to stream via `PowerSyncRemoteClient.pushPack`, keeping pack uploads within the daemon's PowerSync replica and forwarding summary/pack metadata through the shared `options` structure. Local vitest remains blocked by the missing `@tanstack/db` workspace dependency, so tests couldn't be executed.
- Wired Vitest to load `@shared/core` directly from sources and skip the Supabase-dependent git e2e helper suite when the CLI isn't available; refreshed daemon writer tests to assert synced-table upserts instead of legacy `git_packs`/`refs` names so both packages' test suites pass locally.
...
- CLI e2e suite now boots the local Supabase stack via the shared `test-stack-hooks` helper, seeds refs/commits directly into the Postgres tables, and runs the compiled CLI binary so worker paths resolve correctly.
...
- CLI now delegates `psgit sync` to the daemon: we removed the local PowerSync database, call the daemon's new `/summary` endpoint, and print synced-table counts instead of creating a SQLite snapshot. New RPCs (`/summary`) were added to the daemon and shared client; tests/docs updated accordingly.
...
- [ ] Cache auth credentials (Supabase-issued token or service token) securely; refresh automatically. *(Supabase refresh/access tokens now live solely in `supabase-auth.json`; `session.json` keeps only PowerSync metadata while the daemon hydrates from the shared storage and schedules refreshes. Still need to finish the end-to-end automatic refresh orchestration.)*
