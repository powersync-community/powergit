...
async function ensureWorkspaceBinaries() {
  await runCommand('pnpm', ['--filter', '@powersync-community/powergit-remote-helper', 'build']).catch((error) => {
    errorLog('[dev:stack] Failed to build remote helper.', error?.message ?? error)
    throw error
  })

  await runCommand('pnpm', ['--filter', '@powersync-community/powergit', 'build']).catch((error) => {
    errorLog('[dev:stack] Failed to build @powersync-community/powergit.', error?.message ?? error)
    throw error
  })
}
...
function stripPowergitScheme(remoteUrl) {
  if (!remoteUrl) return remoteUrl
  if (remoteUrl.startsWith('powergit::')) {
    return remoteUrl.slice('powergit::'.length)
  }
  return remoteUrl
}
...
function buildStackEnv(statusEnv) {
  ...
  return {
    ...
    powersyncEndpoint,
    remoteUrl,
    remoteHttpUrl: stripPowergitScheme(remoteUrl),
...
function buildExportLines(env, authUser) {
  const lines = [
    ...
    `export POWERGIT_TEST_REMOTE_URL=${JSON.stringify(env.remoteUrl)}`,
    `export POWERGIT_TEST_REMOTE_NAME=${JSON.stringify(DEFAULT_REMOTE_NAME)}`,
    `export POWERGIT_TEST_SUPABASE_URL=${JSON.stringify(env.supabaseUrl)}`,
    `export POWERGIT_TEST_SUPABASE_SERVICE_ROLE_KEY=${JSON.stringify(env.serviceRoleKey)}`,
    `export POWERGIT_TEST_ENDPOINT=${JSON.stringify(env.powersyncEndpoint)}`,
    '',
  ...
  if (authUser) {
    ...
    lines.push(`export POWERGIT_TEST_SUPABASE_EMAIL=${JSON.stringify(authUser.email)}`)
    lines.push(`export POWERGIT_TEST_SUPABASE_PASSWORD=${JSON.stringify(authUser.password)}`)
  }
...
async function syncLocalProfile(env, authUser) {
  ...
  const profileDir = resolve(override && override.trim().length > 0 ? override : resolve(homedir(), '.powergit'))
...
async function startStack() {
  ...
  if (authUser) {
    const authEnv = normalizeEnvValues({
      ...
      POWERGIT_TEST_SUPABASE_EMAIL: authUser.email,
      POWERGIT_TEST_SUPABASE_PASSWORD: authUser.password,
    })
